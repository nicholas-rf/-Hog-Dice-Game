...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
        newLst = [0]
        for number in str(opponent_score):
            newLst.append(int(number))
        scoreAddition = max(newLst)
        return 1 + scoreAddition
    else:
...
def select_dice(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    addedScore = score + opponent_score
    if addedScore % 7 == 0:
        return four_sided
    else:
        return six_sided
    ...
    while score < goal and opponent_score < goal:
        if who == 0:
            if select_dice(score, opponent_score) == six_sided:
                ...
                dice = four_sided
            playerOneScore = take_turn(strategy0(score, opponent_score), opponent_score, dice)
            score += playerOneScore
            if (2*score == opponent_score or score == opponent_score*2):
                temp = score
                score = opponent_score
                opponent_score = temp
            who = other(who)
        elif who == 1:
            if select_dice(opponent_score, score) == six_sided:
                dice = six_sided
            else:
                dice = four_sided
            playerTwoScore = take_turn(strategy1(opponent_score, score), score, dice)
            opponent_score += playerTwoScore
            if (2*opponent_score == score or opponent_score == score*2):
                temp = opponent_score
...
def make_averaged(fn, num_samples=1000):
    ...
    def average_finder(*args):
        numberOfArguments = 0
        sumOfArguments = 0
        for argument in range(num_samples):
            numberOfArguments += 1
            sumOfArguments += fn(*args)
        if sumOfArguments != 0:
            return sumOfArguments / numberOfArguments
        else:
            return 0
    return average_finder
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    i = 0
    for i in range(1, 11):
        diceIAverageFinder = make_averaged(roll_dice, i)
        if maximum > diceIAverageFinder(i, dice):
            i += 1
        else:
            maximum = diceIAverageFinder(i, dice)
            print(i, "dice scores", maximum, "on average")
    return i
...
def bacon_strategy(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(playerScore):
        newLst = [0]
        for number in str(playerScore):
            newLst.append(int(number))
        maxOfScore = max(newLst)
        return maxOfScore + 1
 
    if helper(opponent_score) >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    def helper(playerScore):
        ...
        maxOfScore = max(newLst)
        return maxOfScore + 1
    scoreAdditionIfZero = helper(opponent_score)
    zeroScore = score + scoreAdditionIfZero
    if zeroScore == opponent_score * 2:
        return BASELINE_NUM_ROLLS
    elif zeroScore == opponent_score // 2:
        return 0
    else:
